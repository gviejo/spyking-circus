...
def SpykingCircus2Neurosuite(basepath):
	...
	for i in shank_to_channel.keys():
		...
		for j in shank_to_channel[i]:
			tmp = clusters['clusters_'+str(j)].value		
			if len(tmp):			
				tmp = tmp - tmp.min()
				...
				tmp = tmp + count			
				clu[i].append(tmp)
				res[i].append(clusters['times_'+str(j)].value)
	...
	for i in clu.keys():
		waveforms = np.zeros((len(clu[i])-1,40,8), dtype=np.int16)	
		for j in range(len(res[i])):
			idx = np.searchsorted(timestep, res[i][j])
			waveforms[j] = fp[idx-20:idx+20,shank_to_channel[i]]
		spk[i] = waveforms
